...
def companies(user):
    ...
    return jsonify(companies=[company_json(item) for item in query.order_by(Company.name).all()])
...
def students(user):
    ...
    return jsonify(students=[student_json(item) for item in query.order_by(Student.full_name).all()])
